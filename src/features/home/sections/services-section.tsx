          ...
          {SERVICES.map((service, index) => {
            ...
            return (
              <AnimateOnScroll key={service.id} delay={index * 0.08}>
                <article
                  className={cn(
                    "group relative flex min-h-[320px] flex-col rounded-2xl border border-border bg-white p-7 transition-all duration-300 hover:-translate-y-2 hover:border-primary/30 hover:shadow-[0_20px_60px_rgba(37,99,235,0.18)]"
                  )}
                >
                <div className="absolute inset-x-0 top-0 h-1 rounded-t-2xl bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                  <div className="flex size-16 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:-rotate-1">
